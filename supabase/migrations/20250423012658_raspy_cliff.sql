/*
  ...
  1. Changes
    - Modified 'Master users can manage all users' policy to prevent recursion by using JWT claims
  ...
  2. Security
    ...
    - Fixed recursive policy definitions using JWT claims
  ...
  USING (
    EXISTS (
      ...
      AND coalesce(
        nullif(current_setting('request.jwt.claim.permissao', true), ''),
        'viewer'
      )::user_role IN ('cliente'::user_role, 'viewer'::user_role)
    )
  );

-- Fixed policy to prevent infinite recursion using JWT claims
